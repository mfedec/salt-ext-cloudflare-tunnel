...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/cloudflare_tunnel/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/cloudflare_tunnel/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/cloudflare_tunnel/*")
            ...
            #    "--include=src/saltext/cloudflare_tunnel/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/cloudflare_tunnel/config/schemas",
    )
